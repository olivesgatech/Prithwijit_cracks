...
model.decode_head.classifier = nn.Conv2d(in_channels=model.config.hidden_sizes[-1], out_channels=1, kernel_size=1)
model.to(device)

# Freeze the first half of the encoder layers
for idx, layer in enumerate(model.segformer.encoder.block):
    if idx < len(model.segformer.encoder.block) // 2:  # Freeze early layers
        for param in layer.parameters():
            param.requires_grad = False

# Ensure decoder (segmentation head) is trainable
...
#     return generalized_dice_loss_fp_penalized(masks, outputs)

def criterion(outputs, masks):
    return dice_loss_with_empty_handling(masks, outputs)
#dice_loss_with_empty_handling
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for images, masks in tqdm(val_loader, desc="Validating", leave=False):
            ...
            all_preds.append((outputs > 0.88).cpu().float())  # Binarize predictions
