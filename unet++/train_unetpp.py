...
def log_segmentation_results(images, masks, preds, epoch):
    """
    Logs segmentation results (input, ground truth, prediction) to Weights & Biases.

    Args:
        images (Tensor or NumPy array): Input images.
        masks (Tensor or NumPy array): Ground truth masks.
        preds (Tensor or NumPy array): Model predictions.
        epoch (int): Current training epoch.
    """
    num_samples = min(5, len(images))  # Log up to 5 samples
    fig, axes = plt.subplots(num_samples, 3, figsize=(10, num_samples * 3))

    for i in range(num_samples):
        ax = axes[i] if num_samples > 1 else axes

        # Convert image tensor to NumPy (CxHxW → HxWxC)
        ...
        if isinstance(preds[i], torch.Tensor):
            pred = preds[i].squeeze(0).cpu().numpy()
        else:
            pred = preds[i].squeeze(0)

        ax[0].imshow(img)  # Input image
        ax[0].set_title("Input Image")

        ax[1].imshow(mask, cmap="gray")  # Ground Truth
        ax[1].set_title("Ground Truth")

        ax[2].imshow(pred, cmap="gray")  # Model Prediction
        ax[2].set_title("Model Prediction")

    plt.tight_layout()
    wandb.log({f"Segmentation Results Epoch {epoch}": wandb.Image(fig)})
    plt.close(fig)
...
patience = 5
early_stop_count = 0
checkpoint_path = "./checkpoints/unetpp_resnet50_checkpoint.pth"
best_model_path = "./checkpoints/bestpp_unet_resnet50.pth"
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (images, masks) in enumerate(val_loader_tqdm):
            ...
            #     sample_preds = outputs.cpu()
            # if epoch % 2 == 0:
            #     log_segmentation_results(sample_images, sample_masks, sample_preds, epoch)
